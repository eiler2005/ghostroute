...
Скрипт `domain-auto-add.sh` запускается каждый час через cron — парсит DNS-лог dnsmasq, фильтрует системные домены и российские TLD, добавляет новые в `/jffs/configs/dnsmasq-autodiscovered.conf.add`.
