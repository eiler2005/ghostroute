...
3. **ISP-проба** кандидатов — порог считается по накопленному окну `24h`: обычные кандидаты идут в пробу после `count24h ≥ 3`, а короткие/входные и IP-encoded family домены — с `count24h ≥ 1`. Отбор fair: `12` top-score + `4` кандидата из очереди “давно/никогда не пробовались”. `HTTP 000` → добавляется как **geo-blocked** (сайт блокирует российские IP сам)
