...
3. **ISP-проба** кандидатов — обычные кандидаты проверяются после `≥3` запросов, но короткие входные домены вроде `www.ig.com` приоритизируются и могут проверяться уже после одного запроса. `HTTP 000` → добавляется как **geo-blocked** (сайт блокирует российские IP сам)
4. Записывает **registrable domain** (`example-provider.invalid` вместо `www.example-provider.invalid`) — покрывает все поддомены
