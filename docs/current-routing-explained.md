...
3. **ISP-проба** кандидатов — обычные кандидаты проверяются после `≥3` запросов, но короткие входные домены вроде `www.ig.com` и dynamic DNS хосты с IP-encoded family label приоритизируются и могут проверяться уже после одного запроса. `HTTP 000` → добавляется как **geo-blocked** (сайт блокирует российские IP сам)
4. Записывает **service-family domain** — обычно registrable domain (`example-provider.invalid` вместо `www.example-provider.invalid`), а для dynamic DNS с IP-encoded family label пишет семейство по IP-лейблу
