...
|---|---|---|---|---|---|---|---|
| Routing Core | Production split-routing data plane: DNS classification, ipsets, sing-box REDIRECT, home Reality ingress, Reality egress, direct fallback, Channel M maxtg service egress and the sing-box listener/watchdog scripts. | `modules/routing-core` | `./deploy.sh`, `./modules/routing-core/router/update-singbox-rule-sets.sh`, `ansible-playbook playbooks/23-channel-m-reverse.yml` | router `/jffs/scripts/*`, `/opt/etc/sing-box/rule-sets`, `/jffs/scripts/singbox-watchdog.sh`, VPS docker bridge firewall persistence | `architecture.md`, `modules/routing-core/docs/network-flow-and-observer-model.md`, `modules/routing-core/docs/stealth-channel-implementation-guide.md`, `modules/routing-core/docs/channel-routing-operations.md` | `./verify.sh`, router health fixtures, `tests/test-channel-m-static.sh` | Mutating only during explicit deploy, explicit rule-set refresh, or approved recovery |
...
| Client Profile Factory | Generate, view and clean QR/VLESS profiles for router identity, home-mobile clients, emergency profiles, selected-client Channel B/C artifacts, and the Channel M maxtg service env fragment. | `modules/client-profile-factory` | `./modules/client-profile-factory/bin/client-profiles`, `ansible-playbook playbooks/30-generate-client-profiles.yml` | `ansible/out/clients*`, `ansible/out/channel-m-maxtg` gitignored artifacts | `modules/client-profile-factory/docs/client-profiles.md`, `getting-started.md`, `modules/secrets-management/docs/secrets-management.md` | syntax + secret hygiene checks | Local artifact generation; B/C artifacts are not automatic failover; Channel M env fragments and credentials stay outside git |
