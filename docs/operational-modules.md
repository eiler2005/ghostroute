...
| GhostRoute Console | Caddy-protected factual web console over JSON snapshots, SQLite v4 evidence rows, route explanations, SSE live events and audited controlled actions. | `modules/ghostroute-console` | `./modules/ghostroute-console/bin/ghostroute-console` | local/VPS container data at `ghostroute-console/data` or `/opt/ghostroute-console/data`; VPS app listens on `127.0.0.1:3000` behind Caddy Basic Auth | `modules/ghostroute-console/README.md`, `docs/ghostroute-console-architecture.md`, `docs/ghostroute-console-post-mvp-roadmap.md` | `./modules/ghostroute-console/tests/test-json-contracts.sh`, app `npm test`/`npm run build`/Playwright | Runtime-safe by default; live tail is read-only; catalog apply prepares audited local patches, no hidden router deploy |
