...
| GhostRoute Console | Caddy-protected factual web console over JSON snapshots, route evidence, live events and audited controlled actions. | `modules/ghostroute-console` | `./modules/ghostroute-console/bin/ghostroute-console` | local/VPS container data at `ghostroute-console/data` or `/opt/ghostroute-console/data` | `modules/ghostroute-console/README.md`, `docs/ghostroute-console-post-mvp-roadmap.md` | `./modules/ghostroute-console/tests/test-json-contracts.sh`, app `npm test`/`npm run build`/Playwright | Runtime-safe by default; catalog apply prepares audited local patches, no hidden router deploy |
