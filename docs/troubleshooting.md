...
- **Домен не в реестре РКН и не проходит ISP-пробу** — порог считается по окну `24h`: обычные кандидаты идут в пробу после `count24h ≥ 3`, короткие/`www`-входные — с `count24h ≥ 1`. Если сайт доступен через ISP (HTTP не `000`) — он не добавляется. Добавьте вручную
