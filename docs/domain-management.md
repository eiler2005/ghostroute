...
Remote QR/VLESS-клиенты (`iphone-*`, `macbook`) не зависят от этих ipset для первого hop: они подключаются к домашнему ASUS на `:<home-reality-port>`, попадают в `sing-box` home Reality inbound и затем выходят через существующий Reality outbound на VPS.
