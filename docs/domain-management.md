...
Скрипт `domain-auto-add.sh` запускается каждые 4 часа через cron и автоматически добавляет новые домены в VPN без участия пользователя.

### Как это работает

1. Парсит DNS-лог dnsmasq — смотрит, к каким доменам реально обращались устройства
2. Фильтрует системные домены, CDN-инфраструктуру, российские TLD, домены из `configs/domains-no-vpn.txt`
3. Пропускает домены, уже добавленные вручную или автоматически
4. Добавляет оставшиеся в `/jffs/configs/dnsmasq-autodiscovered.conf.add`
