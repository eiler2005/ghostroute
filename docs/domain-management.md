...
[6] В реестре РКН                           ──нет──► КАНДИДАТ
    (blocked-domains.lst)?                       │
    Если список не скачан → пропустить           │  ≥3 запросов, ≥1 устройство?
    проверку (fallback = добавить всё)           │
          │                                      ▼
          │                              ISP-проба:
...
- `КАНДИДАТЫ` — не в РКН и недостаточно запросов / ISP доступен; добавить вручную если нужно
