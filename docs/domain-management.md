...
1. Парсит DNS-лог dnsmasq — смотрит, к каким доменам реально обращались устройства
2. Фильтрует системные домены, CDN-инфраструктуру, российские TLD, домены из `configs/domains-no-vpn.txt`, уже добавленные
3. **Проверяет по реестру РКН** (`/opt/tmp/blocked-domains.lst`) — домены не из реестра уходят в «кандидаты»
4. **Записывает registrable domain** (`example-provider.invalid` вместо `www.example-provider.invalid`) — одно правило покрывает все поддомены
5. **ISP-проба кандидатов** — при ≥3 запросах от ≥1 устройства делает `curl --interface wan0` (4 сек). HTTP 000 = заблокировано на уровне ISP или geo-block → добавляет как **geo-blocked** без наличия в реестре. До 8 проб за запуск
6. Добавляет прошедшие проверку в `/jffs/configs/dnsmasq-autodiscovered.conf.add`
