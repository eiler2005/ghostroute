...
4. **Пишет service-family domain** — обычно registrable domain (e.g. `example-provider.invalid`) вместо конкретного поддомена (`www.example-provider.invalid`), но для dynamic DNS с IP-encoded family label использует семейство по IP-лейблу, а не весь публичный суффикс.
5. **ISP-проба кандидатов** — обычные домены тестируются после `≥3` запросов, а короткие/входные домены (например `www.ig.com`) и dynamic DNS хосты с IP-encoded family label получают повышенный приоритет и могут проверяться уже после одного запроса. `HTTP 000` (таймаут) = ISP блокирует → добавляются как **geo-blocked** даже без наличия в реестре РКН. Максимум 16 проб за запуск.
