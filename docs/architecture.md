...
3. **Проверяет по списку заблокированных** (`/opt/tmp/blocked-domains.lst`) — добавляет только домены, реально заблокированные в РФ. Если список не скачан — fallback к старому поведению (добавлять всё).
4. Добавляет прошедшие проверку в `/jffs/configs/dnsmasq-autodiscovered.conf.add` (ipset + server entries)
5. Перезапускает dnsmasq
6. Пишет лог в `/opt/var/log/domain-activity.log` (включая секцию «кандидаты» — домены, пропущенные из-за отсутствия в списке блокировок)
7. Ротирует dnsmasq.log
