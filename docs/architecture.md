...
3. **Проверяет по списку заблокированных** (`/opt/tmp/blocked-domains.lst`) — добавляет только домены из реестра РКН. Если список не скачан — fallback (добавлять всё). Домены, не попавшие в список, становятся «кандидатами».
4. **Пишет registrable domain** (e.g. `example-provider.invalid`) вместо конкретного поддомена (`www.example-provider.invalid`) — все поддомены сразу покрываются одним правилом.
5. **ISP-проба кандидатов** — обычные домены тестируются после `≥3` запросов, а короткие/входные домены (например `www.ig.com`) получают повышенный приоритет и могут проверяться уже после одного запроса. `HTTP 000` (таймаут) = ISP блокирует → добавляются как **geo-blocked** даже без наличия в реестре РКН. Максимум 16 проб за запуск.
6. Добавляет прошедшие проверку в `/jffs/configs/dnsmasq-autodiscovered.conf.add` (ipset + server entries)
7. Перезапускает dnsmasq
8. Пишет лог в `/opt/var/log/domain-activity.log` (секции: ДОБАВЛЕНО В VPN, GEO-BLOCKED, КАНДИДАТЫ)
9. Ротирует dnsmasq.log
