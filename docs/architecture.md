...
5. **ISP-проба кандидатов** — домены с ≥3 запросов от ≥1 устройства тестируются через `curl --interface wan0`. HTTP 000 (таймаут) = ISP блокирует → добавляются как **geo-blocked** даже без наличия в реестре РКН. Максимум 8 проб за запуск.
