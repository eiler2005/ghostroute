...
router_collect_capacity_history \
  "${MODULE_DIR}/fixtures/router-health/journal-sample.md" \
  "$HISTORY_OUT" \
  "2026-04-17"
...
router_extract_traffic_summary \
  "${MODULE_DIR}/fixtures/router-health/traffic-report-sample.txt" \
  "$TRAFFIC_OUT"
...
assert_contains "${MODULE_DIR}/fixtures/router-health/traffic-report-sample.txt" "=== TOP BY TAILSCALE PEERS ==="
assert_contains "${MODULE_DIR}/fixtures/router-health/traffic-report-sample.txt" "=== MOBILE HOME REALITY ==="
...
router_render_health_markdown \
  "${MODULE_DIR}/fixtures/router-health/state-sample.env" \
  "$HISTORY_OUT" \
  "$TRAFFIC_OUT" > "$MARKDOWN_OUT"

assert_contains "$MARKDOWN_OUT" "# Router Health Latest"
assert_contains "$MARKDOWN_OUT" "## Routing Health"
assert_contains "$MARKDOWN_OUT" "Home Reality listener"
assert_contains "$MARKDOWN_OUT" "Home Reality IPv4-only listener"
assert_contains "$MARKDOWN_OUT" "Home Reality INPUT allow"
assert_contains "$MARKDOWN_OUT" "Home Reality connlimit before ACCEPT"
assert_contains "$MARKDOWN_OUT" "Home Reality LTE MSS clamp"
assert_contains "$MARKDOWN_OUT" "Router TCP high-BDP tuning"
assert_contains "$MARKDOWN_OUT" "Home Reality DNS -> dnsmasq"
assert_contains "$MARKDOWN_OUT" "Managed VPS DNS include"
assert_contains "$MARKDOWN_OUT" "vps-dns-in listener/rule"
assert_contains "$MARKDOWN_OUT" "LAN DNS capture :53"
...
assert_contains "$MARKDOWN_OUT" "Client observed total:"
...
router_render_health_markdown \
  "${MODULE_DIR}/fixtures/router-health/state-ipv6-partial.env" \
  "$HISTORY_OUT" \
...
router_render_health_markdown \
  "${MODULE_DIR}/fixtures/router-health/state-ipv6-runtime.env" \
  "$HISTORY_OUT" \
