...
[ "$(router_kv_get "$STATE_FILE" HOME_REALITY_DNS_GUARD_RULE)" = "1" ] || add_critical "home Reality ingress missing plain DNS -> router dnsmasq rule"
[ "$(router_kv_get "$STATE_FILE" MANAGED_VPS_DNS_INCLUDE)" = "1" ] || add_critical "managed VPS DNS include is missing"
[ "$(router_kv_get "$STATE_FILE" VPS_DNS_FORWARD_LISTENER)" = "1" ] || add_critical "vps-dns-in listener missing on router loopback"
[ "$(router_kv_get "$STATE_FILE" VPS_DNS_FORWARD_RULE)" = "1" ] || add_critical "vps-dns-in is not routed to reality-out"
[ "$(router_kv_get "$STATE_FILE" LAN_DNS_CAPTURE_UDP)" = "1" ] || add_critical "LAN UDP/53 is not captured by router dnsmasq"
[ "$(router_kv_get "$STATE_FILE" LAN_DNS_CAPTURE_TCP)" = "1" ] || add_critical "LAN TCP/53 is not captured by router dnsmasq"
...
printf "%-34s %s\n" "Home Reality DNS -> dnsmasq" "$( [ "$(router_kv_get "$STATE_FILE" HOME_REALITY_DNS_GUARD_RULE)" = "1" ] && echo OK || echo MISSING )"
printf "%-34s %s\n" "managed VPS DNS include" "$( [ "$(router_kv_get "$STATE_FILE" MANAGED_VPS_DNS_INCLUDE)" = "1" ] && echo OK || echo MISSING )"
printf "%-34s %s\n" "vps-dns-in listener/rule" "$( [ "$(router_kv_get "$STATE_FILE" VPS_DNS_FORWARD_LISTENER)" = "1" ] && [ "$(router_kv_get "$STATE_FILE" VPS_DNS_FORWARD_RULE)" = "1" ] && echo OK || echo MISSING )"
printf "%-34s %s\n" "LAN DNS capture :53" "$( [ "$(router_kv_get "$STATE_FILE" LAN_DNS_CAPTURE_UDP)" = "1" ] && [ "$(router_kv_get "$STATE_FILE" LAN_DNS_CAPTURE_TCP)" = "1" ] && echo OK || echo MISSING )"
...
printf "%-34s %s\n" "dnscrypt SOCKS listener" "$( [ "$(router_kv_get "$STATE_FILE" CHANNEL_B_DNSCRYPT_SOCKS_LISTENER)" = "1" ] && echo OK || echo MISSING )"
