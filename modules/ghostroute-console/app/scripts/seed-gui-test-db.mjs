...
function seed(db) {
  ...
  fs.writeFileSync(
    path.join(dataDir, "policy-snapshot.local.json"),
    `${JSON.stringify({
      schema_version: 1,
      generated_at: iso(now, 250),
      source: { command: "test-policy-seed", mode: "sanitized" },
      confidence: "exact",
      home_wifi_lan_full_vps: [
        {
          id: "test-home-laptop",
          name: "test-home-laptop",
          label: "Test/Home Laptop",
          selector: "reserved_source_ip",
          ip: "192.0.2.44",
          ip_token: "ip-0f0a4411",
          mac: "02:00:5e:10:00:44",
          mac_token: "mac-0f0a4411",
          interface: "br0",
          strict_dns_status: "configured",
          status: "enabled",
          full_vps: true,
        },
        {
          id: "test-iphone-heavy",
          name: "test-iphone-heavy",
          label: "Test/iPhone Heavy",
          selector: "reserved_source_ip",
          ip_token: "ip-0f0a5151",
          mac_token: "mac-0f0a5151",
          interface: "br0",
          strict_dns_status: "configured",
          status: "enabled",
          full_vps: true,
      ...
      channel_profiles: [
        {
          channel: "A",
          profile: "test-channel-a-full",
          label: "Test/Channel A Full",
        ...
        {
          channel: "A",
          profile: "test-channel-a-split",
          label: "Test/Channel A Split",
          profile_type: "home_reality",
          status: "enabled",
        ...
        {
          channel: "B",
          profile: "test-channel-b",
          label: "Test/Channel B",
        ...
        {
          channel: "C",
          profile: "test-channel-c",
          label: "Test/Channel C",
  ...
  const counterOnlyClientFlows = [
    {
      id: "test-seed:counter-only-large",
      client: "mobile-source-99",
      client_ip: "192.0.2.88",
      device_key: "mobile-source-99",
      channel: "Unknown",
      route: "Mixed",
      bytes: 2_500_000,
      via_vps_bytes: 1_400_000,
      direct_bytes: 900_000,
    },
    {
      id: "test-seed:counter-only-raw-large",
      client: "192.0.2.90",
      client_ip: "192.0.2.90",
      device_key: "192.0.2.90",
      channel: "Home Wi-Fi/LAN",
      route: "Direct",
      bytes: 1_600_000,
      via_vps_bytes: 0,
      direct_bytes: 1_600_000,
    },
    {
      id: "test-seed:counter-only-noise",
      client: "192.0.2.89",
      client_ip: "192.0.2.89",
      device_key: "192.0.2.89",
      channel: "Home Wi-Fi/LAN",
      route: "Direct",
      bytes: 700_000,
      via_vps_bytes: 0,
      direct_bytes: 700_000,
    },
  ];
  for (const [idx, flow] of counterOnlyClientFlows.entries()) {
    normalizedFlowStmt.run({
      snapshot_id: trafficSnapshotId,
      snapshot_type: "traffic_facts",
      collected_at: iso(now, 39000 + idx * 2000),
      client: flow.client,
      ...
      dns_answer_ip: "",
      sni: "",
      outbound: flow.route === "VPS" ? "reality-out" : flow.route === "Direct" ? "direct" : "mixed",
      matched_rule: "",
      rule_set: "",
      source_log: "traffic-evidence",
      traffic_class: "client",
      via_vps_bytes: flow.via_vps_bytes,
      direct_bytes: flow.direct_bytes,
      unknown_bytes: Math.max(0, flow.bytes - flow.via_vps_bytes - flow.direct_bytes),
      route_verification: flow.route === "Mixed" ? "counter_allocated" : "counter_only",
      route_status: flow.route === "Mixed" ? "counter_allocated" : "counter_only",
      raw_json: JSON.stringify({
        fact_id: flow.id,
        schema_version: 3,
        evidence_level: "counter_only",
        byte_confidence: "counter_exact_destination_missing",
        accounting_status: "ok",
      }),
    });
  }
  ...
  const heavyDnsRows = [
    ["test-iphone-heavy", "gs-loc.apple.com", 15],
    ["test-iphone-heavy", "gs-loc.ls-apple.com.akadns.net", 14],
    ["test-iphone-heavy", "youtubei.googleapis.com", 13],
    ["test-iphone-heavy", "api.telegram.org", 12],
    ["test-iphone-heavy", "instagram.com", 11],
    ["test-iphone-heavy", "graph.instagram.com", 10],
    ["test-iphone-heavy", "api.spotify.com", 9],
    ["test-iphone-heavy", "music.apple.com", 8],
    ["test-iphone-heavy", "icloud.com", 7],
    ["test-iphone-heavy", "gsp-ssl.ls.apple.com", 6],
    ["test-iphone-heavy", "maps.googleapis.com", 5],
    ["test-iphone-heavy", "api.openai.com", 4],
    ["test-iphone-heavy", "chatgpt.com", 4],
    ["test-iphone-heavy", "netflix.com", 3],
    ["test-iphone-heavy", "cdn.discordapp.com", 3],
    ["test-iphone-heavy", "github.com", 2],
    ["test-macbook-heavy", "graph.instagram.com", 16],
    ["test-macbook-heavy", "chatgpt.com", 15],
    ["test-macbook-heavy", "api.openai.com", 14],
    ["test-macbook-heavy", "github.com", 13],
    ["test-macbook-heavy", "api.github.com", 12],
    ["test-macbook-heavy", "cloudflare.com", 11],
    ["test-macbook-heavy", "youtube.com", 10],
    ["test-macbook-heavy", "youtubei.googleapis.com", 9],
    ["test-macbook-heavy", "slack.com", 8],
    ["test-macbook-heavy", "notion.so", 7],
    ["test-macbook-heavy", "dropbox.com", 6],
    ["test-macbook-heavy", "apple.com", 5],
    ["test-macbook-heavy", "icloud.com", 4],
    ["test-macbook-heavy", "telegram.org", 3],
    ["test-macbook-heavy", "cdn.discordapp.com", 2],
  ];
