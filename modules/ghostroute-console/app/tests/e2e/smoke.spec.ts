...
test("mobile redirect preserves bypass and safe routes", async ({ page, request, isMobile }) => {
  ...
  const forwardedAliasRedirect = await request.get("/traffic", {
    headers: {
      host: "127.0.0.1:3000",
      "sec-ch-ua-mobile": "?1",
      "user-agent":
  ...
  expect(forwardedAliasRedirect.status()).toBe(307);
  expect(forwardedAliasRedirect.headers().location).toBe("https://console.example.invalid/m/traffic");
