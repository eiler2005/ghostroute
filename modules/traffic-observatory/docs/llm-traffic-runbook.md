...
8. If the report contains "USERS / APPS BY CHANNEL", use it as the primary
   answer for questions about users, channels, and applications:
   - for LAN/Wi-Fi users, quote exact `Total`, `VPS`, and `Direct` bytes and
     treat app families as DNS-interest counts, not per-app byte totals
   - for Channel A/Home Reality users, quote estimated `Traffic`, `VPS est.`,
     `Direct est.`, connection path split, and top app families
   - for Channel B/C users, quote profile/user and app connection counts; say
     byte attribution is not available for those lanes yet

9. If the report contains "DEVICE TRAFFIC MIX", use that block first for per-device interpretation:
...
10. If the report contains "HOME REALITY INGRESS CLIENTS", include Home Reality ingress activity:
...
12. If the report contains "TOP BY TAILSCALE PEERS", mention the busiest remote peers before the full peer table.

13. If the report contains "LAN DEVICE BYTES", use it for exact per-device byte numbers.
    If the report contains only "LAN DEVICES", explain that these are active conntrack counts, not bytes.

14. If the report is from ./modules/traffic-observatory/bin/traffic-daily-report, answer from its period sections:
...
15. If the report window is week/month, explicitly warn when any sample window is narrower than the nominal period.

16. Never expose private router IPs, client private IPs, MAC addresses, SSH keys, raw endpoints, or unredacted device names unless the user explicitly asks for trusted local inspection.

17. If the user asks about future optimization of domains/catalog/routing coverage:
