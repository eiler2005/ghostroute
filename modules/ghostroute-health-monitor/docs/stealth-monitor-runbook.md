...
включая endpoint-пробы `hermes_vps`, когда он сконфигурирован
(`hermes_tcp` / `hermes_tls`), проверяет application canaries через router SOCKS
и не печатает реальные endpoint/IP/provider значения. В backup mode падение `primary_tls` остаётся
`WARN`, пока active canaries зелёные; switchback на primary делай только после
нескольких зелёных `primary_tls`.

`egress-backend-health` используй, когда нужен именно backend-bank с ролями
`primary_vps`, `backup_reality`, `hermes_vps`, а не только текущий active check.
Команда читает безопасный TSV с Vault references, делает router-side TCP/TLS
probe для каждой роли и отдельно гоняет application canaries через активный
SOCKS. TCP/TLS для inactive backend-ов являются advisory; canonical proof для
текущего backend-а - app canaries. Вывод human/JSON не должен содержать реальные
endpoint, IP, provider, UUID, key или SNI значения.
...
`status`, `leak-check`, `managed-egress-check` и `egress-backend-health` не
рестартят сервисы, не меняют `iptables`/`ipset`, не запускают mutating Ansible и
не трогают secrets. `leak-check` может дописать свежие probe events в
health-monitor storage, потому что это monitoring-owned evidence, а не
