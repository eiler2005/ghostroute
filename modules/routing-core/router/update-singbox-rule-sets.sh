...
for runtime_env in /jffs/scripts/ghostroute-runtime.env /jffs/scripts/ghostroute-runtime-env; do
...
MANAGED_DNS_FORWARD_HOST="${MANAGED_DNS_FORWARD_HOST:-${GHOSTROUTE_MANAGED_DNS_FORWARD_HOST:-127.0.0.1}}"
MANAGED_DNS_FORWARD_PORT="${MANAGED_DNS_FORWARD_PORT:-${GHOSTROUTE_MANAGED_DNS_FORWARD_PORT:-${GHOSTROUTE_DNSCRYPT_PORT:-5354}}}"
# Backward-compatible overrides for older operator scripts.
VPS_DNS_FORWARD_HOST="${VPS_DNS_FORWARD_HOST:-$MANAGED_DNS_FORWARD_HOST}"
VPS_DNS_FORWARD_PORT="${VPS_DNS_FORWARD_PORT:-$MANAGED_DNS_FORWARD_PORT}"
...
write_dnsmasq_vps_dns() {
...

  {
    ...
    printf '# Forward target is dnscrypt-backed by default; dnscrypt uses sing-box SOCKS/Reality.\n'
...
echo "dnsmasq managed DNS: domains=${vps_dns_domain_count}, changed=${DNSMASQ_CHANGED}, target=${DNSMASQ_VPS_DNS_CONF}, forward=${VPS_DNS_FORWARD_HOST}#${VPS_DNS_FORWARD_PORT}"
...
if [ "$MODE" = "--restart-if-changed" ] && [ "$DNSMASQ_CHANGED" = "1" ]; then
  $DNSMASQ_RESTART
  echo "dnsmasq restarted after managed DNS include update"
fi
