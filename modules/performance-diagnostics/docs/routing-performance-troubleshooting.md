...
- Router-side `reality-in` plain DNS `53` is sent to router-local dnsmasq.
  dnsmasq applies policy-based DNS: managed/foreign names use the generated
  managed include pointing at dnscrypt-over-Reality, while RU/direct/default
  names use the home resolver path.
