...
- Layer 2 — home router. Он завершает home-based channels и применяет managed
  ...
  selected full-VPS override для выбранных устройств/профилей.
...
Байты Home Reality профилей входят в trusted pipeline только как read-only
encrypted ingress counter deltas (`home_reality_samples` ->
`Home Reality ingress` facts); Console не возвращает legacy estimated
Apple/iCloud-style destination allocation без реального DNS/flow evidence.
