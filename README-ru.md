...
Роутер каждые 4 часа анализирует все DNS-запросы от устройств в сети и автоматически добавляет в VPN новые заблокированные домены.

Но не все подряд — только те, которые есть в [кураторском списке заблокированных доменов](https://community.antifilter.download). Это защищает от случайного добавления в VPN всего подряд (российских сайтов, которые работают без VPN).

```
dnsmasq.log (все DNS-запросы)
       │
       │  domain-auto-add.sh (каждые 4 часа)
       ▼
 Извлечь уникальные домены
       ...
       └─ Проверить по blocked-domains.lst
             ...
             └─ НЕТ → записать как кандидат (не добавлять)
```
...
- **Авто-обнаружение** — каждые 4 часа находит и добавляет новые заблокированные домены
...
scripts/
  ...
  domain-auto-add.sh              # авто-обнаружение доменов (cron, каждые 4ч)
...
| Авто-обнаружение доменов (каждые 4ч) | Активно |
