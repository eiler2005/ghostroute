...
В текущей production-схеме есть один основной канал и два ручных fallback:

- Channel A — основной быстрый путь:
...
- Channel B — ручной `VLESS+XHTTP+TLS` на отдельном Xray backend.
- Channel C — ручной `NaiveProxy` через Caddy `forward_proxy`.
...
- Опциональные ручные Channel B fallback-профили: VLESS+XHTTP+TLS на отдельном
  public hostname поверх VPS `:443`.
- Опциональный ручной Channel C fallback-профиль: NaiveProxy на отдельном
  public hostname поверх VPS `:443`.
...
Operational layer:
  ...
  Client Profiles     -> QR/VLESS и Channel B/C profiles generation from Vault
...
Channel B и Channel C — ручные fallback для отдельных устройств, а не
изменение router data plane. Профили в `ansible/out/clients-channel-b/` и
`ansible/out/clients-channel-c/` позволяют вручную тестировать
VLESS+XHTTP+TLS или NaiveProxy через отдельные VPS hostnames на том же public
`:443`. В v1 нет auto failover и нет per-domain routing на роутере.
