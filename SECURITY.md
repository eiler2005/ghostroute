...
| Console DB corruption | Quarantine `data/ghostroute.db*`; restart with empty DB; collector re-populates within minutes | <30 min to baseline data; full 7-day backfill on next normal cycle | `vps/deploy-readonly.yml`; `GHOSTROUTE_DB_BACKUP_MODE=daily` |
